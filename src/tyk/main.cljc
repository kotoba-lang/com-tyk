(ns tyk.main
  "Kotodama WASM entrypoint for the Tyk clean-room actor (L4) — Clojure port.
  ...
  facts (`tyk.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"tyk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tyk-compat" :tier tier :entities entities} 200])
